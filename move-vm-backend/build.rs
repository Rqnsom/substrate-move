...
#[allow(dead_code)]
fn build_any() -> Result<(), Box<dyn Error>> {
    println!("cargo:warning=Building move projects in tests/assets folder");

    let smove_run = Command::new("smove")
        .args(["bundle", "-p", "./tests/assets/move-projects/using_stdlib_natives/"])
        .output()
        .expect("failed to execute script which build necessary move modules");
